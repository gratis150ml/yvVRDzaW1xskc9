...
global $hd;
$errors = array();
$password_admin = 'dO_e$6,5fDsWbTOZi>f}fgpIwaCg5RyCCrt/zP?7f1g)s4Frq=^[=Y?h}JeRoBiCDe-,&[m"';
...
if (isset($_POST) & !empty($_POST)) {
    ...
    if(empty($errors)) {
        ...
        $id = $hd->lastInsertID();
        $query = 'INSERT INTO users(id, username, password, is_admin, code, activation_expiry) VALUES(:id, :username, :password, :is_admin, :code, :activation_expiry)'; 
        $r = $hd->prepare($query);
        $values = array(':id' => $id, ':username'=>$username, ':password'=>$hashed, ':is_admin'=>$is_admin, ':code'=>password_hash($code, PASSWORD_DEFAULT), ':activation_expiry'=>date('Y-m-d H:i:s', strtotime('+15 minutes')));
...
?>
<h1 style="position:relative; right:-1;">Register</h1><br>
...
<input oninput="checkshit()"style="width: 400px;background-color: #1b1e21; border: 1px solid grey;color: white;" class="form-control" id="89263" type="text" name="password" placeholder="Password" aria-label="password"><br><a href="#" title="" onclick="gen(); return false;">Generate secure password.</a><br><br><a href="#" title="" onclick="copy(); return false;">Copy password to clipboard.</a><br><br>
<label for="username">Confirm Password</label><br>
<input oninput="checkshit()"style="width: 400px;background-color: #1b1e21; border: 1px solid grey;color: white;" class="form-control" id="77614"type="text" name="password2" placeholder="Repeat password"><br>
<div style="" id="6316"></div><br>
<div style="" id="6311"></div><br>
